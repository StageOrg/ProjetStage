...
const etudiantNotesService = {
  ...
  /**
   * Récupère les UEs avec notes en utilisant le nouvel endpoint dédié
   */
  getMyUEsWithNotes: async () => {
    try {
      // Utiliser le nouvel endpoint dédié pour les étudiants
      ...
      if (!response.data || response.data.length === 0) {
        console.log("Aucune UE avec notes trouvée via l'endpoint dédié");
        return [];
      }
      ...
      console.log(`Données formatées: ${formattedData.length} UEs`);
  ...
  /**
   * Récupère uniquement les UEs sans les notes
   */
  getMyUEsOnly: async () => {
    try {
  ...
  /**
   * Récupère les détails d'une UE spécifique
   */
  getUEDetails: async (ueId) => {
    try {
      // Utiliser l'endpoint dédié et filtrer par UE
      const uesResponse = await api.get("/utilisateurs/etudiants/mes-ues-avec-notes/");
      const uesData = uesResponse.data;
      
      // Trouver l'UE spécifique
      const ueDetails = uesData.find(ue => ue.id === parseInt(ueId));
      
      if (!ueDetails) {
        throw new Error("UE non trouvée dans vos inscriptions");
      }
  ...
  /**
   * Récupère les notes détaillées d'une UE
   */
  ...
  /**
   * Vérifie si l'étudiant a des données académiques
   */
  ...
  /**
   * Récupère un résumé rapide des notes
   */
